...
# Set knitr options for all code chunks in the file.
...
loans$IncomeRange[loans$IncomeRange=="Not employed"] = "$0"
...
```{r Dataset_preview}
preview_col <- c("LoanOriginalAmount", "LoanStatus", "BorrowerAPR",
                 "LenderYield", "EstimatedLoss")
knitr::kable(head(loans[,preview_col]))
loans <- dplyr::select(loans, Term, LoanStatus, ClosedDate, 
...
```{r Univariate_Plots, fig.path='fig/'}
targetCols <- c("BorrowerAPR",
                "LenderYield",
                "EstimatedReturn",
                "EstimatedLoss")

plotHist <- function(x) {
  # plot histogram for variable whos name is given as string x.
  ggplot(loans, aes_string(x)) +
    geom_histogram(binwidth = 0.005)
}

lapply(targetCols, plotHist)
```
...
```{r echo=FALSE, Define_past_loans}
# the date dataset was last updated
last_updated <- as.POSIXct("03/11/2014", format = "%d/%m/%Y")
# subset loans that should have been completed
loans$PlannedClosing <- loans$Date + 
  as.difftime(loans$Term*365/12, units = "days")
past_loans <- subset(loans, PlannedClosing < last_updated)
# simple measure for the loan status, logical
...
```{r DebtToIncomeRatio_histudaci}
ggplot(data = loans, aes(x = DebtToIncomeRatio)) + 
  geom_density() + xlim(0, 1.5)

ggplot(data = loans, aes(x = DebtToIncomeRatio)) + 
  geom_density() + 
  geom_bar(binwidth = 0.02, fill = "blue", alpha = 0.5, aes(y = ..density..)) +
  scale_x_sqrt(limits = c(0, 1.5))

print('Highest frequency Debt To Income Ratios:')
...
below `r quantile(loans$DebtToIncomeRatio, 0.95, na.rm = T)`
...
```{r Correlation_Matrix, fig.height=8, cache=TRUE, cache.path = 'cache/'}
numeric_cols <- c("BorrowerAPR", "BorrowerRate", "LenderYield",
                  ...
                  "MonthlyLoanPayment", "Year", "ProsperReturn")
M <- cor(loans[, numeric_cols], 
         use="pairwise.complete.obs") # ignore missing values

show_cols <- c("BorrowerAPR", "LenderYield", 
               "EstimatedLoss", "EstimatedReturn", "ProsperScore")
knitr::kable(round(M[,show_cols],2))

corrplot(M, method="color", # method = "number"
         type="upper", 
         order="hclust", # arrange by similarity
         tl.srt=60) 

# loans_numeric <- as.data.frame(lapply(loans, as.numeric))
# M <- cor(loans_numeric, use="pairwise.complete.obs")
...
```{r echo=FALSE, warning=FALSE, EstimatedLoss_vs_CreditScore}
ggplot(data = loans, 
     aes(x = CreditScoreRangeUpper, y = EstimatedLoss)) + 
  geom_line(stat='summary', fun.y = mean) +
  geom_line(stat = 'summary', fun.y = quantile, probs=0.025, linetype = 2) +
  geom_line(stat = 'summary', fun.y = quantile, probs=0.975, linetype = 2) +
  scale_y_log10()
...
print("Correlation between IsCompleted and CreditScoreRangeUpper:")
with(subset(past_loans, !is.na(CreditScoreRangeUpper)),
     cor(CreditScoreRangeUpper, IsCompleted))

print("Correlation between IsCompleted and EstimatedLoss:")
with(subset(past_loans, !is.na(EstimatedLoss)),
     cor(EstimatedLoss, IsCompleted))

print("Correlation between IsCompleted and ProsperScore:")
with(subset(past_loans, !is.na(EstimatedLoss)),
     cor(ProsperScore, IsCompleted))
```
...
```{r}
ggplot(data = past_loans_full, aes(x = IsCompleted, y = EstimatedLoss)) +
  geom_boxplot()
with(past_loans_full, cor.test(EstimatedLoss, as.numeric(IsCompleted)))
```
...
```{r Estimated_Return_and_Lost_Separate}
ggplot(aes(x = EstimatedLoss, y=LenderYield), data = loans) +
  geom_point(alpha=1/10, size=2, position = 'jitter') + ylim(c(0, 0.35))+
...
ggplot(aes(x = round(EstimatedLoss/4,digits = 2)*4,  # reducing the resolution
           y=LenderYield),
       data = loans) +
  ...
  geom_line(stat = 'summary', fun.y = quantile, probs=0.95,
    linetype = 2, color = 'blue', alpha=1/2) +
  geom_line(aes(y=EstimatedReturn),  # adding the second variable to explore
            alpha=1/2, size = 2, color = 'red',
...
```{r LenderYield_Year, fig.width=11}
ggplot(data = loans, 
...
```{r ProsperReturn_Year, fig.width=11}
ggplot(data = loans, 
...
```{r CompletionRate_by_Prosper_and_Credit_Score}
completed_groups <- past_loans %>%
  select(CreditScoreRangeUpper, ProsperScore, IsCompleted) %>%
  group_by(CreditScoreRangeUpper, ProsperScore) %>%
  summarize(PercentCompleted = mean(IsCompleted),
            n = n())

ggplot(aes(x = CreditScoreRangeUpper, y = ProsperScore), 
      data = completed_groups) +
  geom_tile(aes(fill = PercentCompleted)) +
  xlim(600, 800) +
  scale_fill_continuous(low = "red", high = "green") +
  scale_color_brewer() +
  geom_smooth(data = past_loans, aes(color = IsCompleted), size=2)
```
...
```{r CompletionRate_by_Prosper_and_Credit_Score_with_SampleSize}
ggplot(aes(x = CreditScoreRangeUpper, y = ProsperScore), 
      data = subset(completed_groups, !is.na(ProsperScore))) +
  geom_point(aes(size = log10(n), color = PercentCompleted)) +
...
```{r EstimatedLoss_multiplot}
qplot(x = CreditScoreRangeUpper, y = EstimatedLoss, color = factor(ProsperScore), 
      data = loans, alpha = 1/2, position = 'jitter') +
...
```{r linear_model}
m1 <- lm(I(log10(EstimatedLoss)) ~ ProsperScore, data = loans)
...
```{r DebtToIncomeRatio}
ggplot(data = subset(loans, DebtToIncomeRatio<3), 
       aes(x = DebtToIncomeRatio, y = EstimatedLoss)) + 
  geom_point(alpha = 1/50) + geom_smooth()
```

Strong influence has also the `Year` - estimated losses are reduced with time.

```{r EstimatedLoss_multiplot_smooth}
ggplot(data = subset(loans, Year>=2009),
...
```{r Bivariate_Plots_explore, fig.width=12,  fig.height=10}

plotHist <- function(x) {
  # plot histogram for variable whos name is given as string x.
  ggplot(subset(loans, Year >=2009 & ProsperRating..Alpha. != "NA"),
         aes_string(x)) +
    geom_histogram(binwidth = 0.005, aes(fill = factor(Term))) +
    scale_y_sqrt() +
    facet_grid(ProsperRating..Alpha.~Year)
}

lapply(c(targetCols, "ProsperReturn"), plotHist)
```
...
grid.arrange(g1, g2, g3, ncol = 3, 
             top = 'Uncompleted loans ratio vs different risk measures') 
```
...
ggplot(aes(x = round(EstimatedLoss/4,digits = 2)*4, y=LenderYield),
  ...
  geom_line(stat = 'summary', fun.y = quantile, probs=0.95,
    linetype = 2, color = 'blue', alpha=1/2) +
  geom_line(aes(y=EstimatedReturn, color = 'Mean estimated return'), 
            alpha=1/2, size = 2,
            stat='summary', fun.y = mean)+
  geom_line(aes(y=EstimatedReturn), color = 'red', alpha=1/2,
            stat='summary', fun.y = median, linetype = 2) +
...
### Plot Three
```{r Plot_Three, fig.width=10, fig.height=8}

ggplot(aes(x = CreditScoreRangeUpper, y = ProsperScore), 
      data = subset(completed_groups, !is.na(ProsperScore) & n>10)) +
  geom_point(aes(size = log10(n), color = PercentCompleted)) +
  scale_size(range = c(0,20), 
             guide = guide_legend(title = 'Samples, log10')) +
  xlim(600, 900) +
  scale_color_continuous(low = "red", high = "green",
                         guide = guide_legend(title = 'Completion rate')) +
  theme_bw() +
  xlab('Credit Score') +
  ylab('Prosper Socre') +
  # we continue on the full dataset, relationship between the risk measures
  # for completed and not completed loans
  geom_smooth(data = past_loans, color = 'black', alpha = 0.5,
              aes(linetype = IsCompleted)) +
  ggtitle('Loan completion ratio as a function of credit risk measures') +
  guides(linetype=guide_legend(title='Completion status'))
```
...
NOTE: Our conclusions are based on past data and will not be very representative of the curre nt situation because the Prosper system is changing in years.
